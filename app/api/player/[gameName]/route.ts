...
import { getRankedSnapshot, getPlayerStats, getMatchesByPuuid } from "@/app/lib/db";
import { getUserByRiotId } from "@/app/data/users";
import type { Summoner, LeagueEntry, PlayerAggregatedStats, Match } from "@/app/types/riot";
...
export async function GET(
  ...
  const snap = getRankedSnapshot(user.puuid);
  if (!snap) {
  ...
  const statsRow = getPlayerStats(user.puuid);
